...
public interface IUserReponsitory extends JpaRepository<UserEntity, String> {
    ...
    @Query(value = "select s from UserEntity s where s.role = '" + ContainsUtils.ROLE_STAFF
            + "' or s.role = '" + ContainsUtils.ROLE_ADMIN
            + "' or s.role = '" + ContainsUtils.ROLE_SUPPER_ADMIN + "'")
